...
class Epoch:
    ...
    def compute_normalized_cross_correlation(self, neuron_1, neuron_2):
        ...
        for lag_time_index, lag_time in enumerate(lags):
            ...
            shifted_neuron_1_spikes = np.zeros(self.n_frames)
            if lag_time == 0:
        ...
        if np.sum(cross_correlation_array) > 0:
            # print(f"{np.min(cross_correlation_array)}")
            normalized_cross_correlation_array = cross_correlation_array / np.sum(cross_correlation_array)
            # if np.max(normalized_cross_correlation_array) > 1:
            #     print(f"max of cross correlation array is {np.max(normalized_cross_correlation_array)}")
        else:
            normalized_cross_correlation_array = cross_correlation_array

        show_corr_bar_chart = False
        save_corr_bar_chart = True
...
def signature_emd_JD_RD(x, y):
    ...
    while q < Q:
        ...
        if weights_x[q] < 0.00005:
            weights_x[q] = 0
            q = q + 1
        if weights_y[r] < 0.00005:
            weights_y[r] = 0
            r = r + 1
        if weights_x[-1] == 0 or weights_y[-1] == 0:
            break
    return emd / (t1 + t2 + 1)
...
def compute_spotdis_between_2_epochs(distance_metric, epoch_1, epoch_2):
    ...
    if total_count > 0:
        ...
        spotdis_value = 1
        # spotdis_value = np.Inf
    return spotdis_value


def SPOT_Dist_JD_RD(data, len_epoch=100, distance_metric="EMD_Battaglia"):
    """"
